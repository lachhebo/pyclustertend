...
def hopkins(D, sampling_size):
    ...
    matrix = np.column_stack((np.random.uniform(min_D[0],max_D[0], sampling_size),np.random.uniform(min_D[1],max_D[1], sampling_size)))
    if len(max_D)>=2:
        for i in range(2,len(max_D)):
            matrix = np.column_stack((matrix,np.random.uniform(min_D[i],max_D[i],sampling_size)))
